...
\date   11/17/2016
\brief  Parsing code adapted from glTF loader example from
\brief https://github.com/syoyo/tinygltfloader/blob/master/loader_example.cc
 
 \brief Parsing code adapted from GPU Programming, University of Pennsylvania 2016,
 \brief https://github.com/CIS565-Fall-2016/Project4-CUDA-Rasterizer
...
void generateSceneAttributes(const std::string& fileName, SSceneAttributes& outScene)
{
	...
	for (auto& nodeString : nodeString2Matrix)
	{
		...
		for (auto& meshName : node.meshes)
		{
			...
			for (size_t i = 0; i < mesh.primitives.size(); i++)
			{
				...
				outScene.m_geometriesData.push_back(SGeometryAttributes());
				SGeometryAttributes& geom = outScene.m_geometriesData.back();
				...
				// -------- Indices ----------
				{
					...
					SVertexAttributes attributeInfo = {
				...
				for (auto& attribute : primitive.attributes)
				{
					...
					geom.m_vertexAttributes.insert(std::make_pair(attributeType, attributeInfo));
					geom.m_vertexData.insert(std::make_pair(attributeType, data));
